...
// Else return all orders filtered by req. userId
...
export async function getOrder(req: Request, res: Response) {
    try {
        ...
        const orderWithItems = await db.select().from(ordersTable).where(eq(ordersTable.id, id))
       ...
       const mergedOrder = {
        ...
        itmes: orderWithItems.map((oi)=>oi.order_items),
       }
